...
public sealed class AvoirsController(
    ...
    IClientService clients) : Controller
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await avoirs.GetAvoirsAsync(
            search,
            page,
            AvoirListViewModel.DefaultPageSize,
            cancellationToken);

        return View(new AvoirListViewModel
        {
            Items = result.Items,
    ...
    public async Task<IActionResult> Create(CancellationToken cancellationToken)
    {
        ...
        return View("Edit", new AvoirFormViewModel
        {
            Numero = numero,
            Date = DateTime.Today,
        });
    ...
    private async Task ResolveClientNomAsync(AvoirFormViewModel model, CancellationToken cancellationToken)
    {
        if (model.ClientId <= 0)
            return;
    ...
    private async Task<AvoirFormViewModel> ToFormViewModelAsync(
        AvoirClientDto dto,
        CancellationToken cancellationToken)
    {
        var client = await clients.GetClientByIdAsync(dto.ClientId, cancellationToken);

        return new AvoirFormViewModel
        {
            Id = dto.Id,
            Numero = dto.Numero,
            ...
            Lignes = dto.Lignes.Select(l => new AvoirLigneViewModel
            {
    ...
    private static CreateAvoirClientDto ToCreateDto(AvoirFormViewModel model) =>
        new(
            model.Numero.Trim(),
            model.ClientId,
    ...
    private static UpdateAvoirClientDto ToUpdateDto(AvoirFormViewModel model) =>
        new(
            model.ClientId,
            NormalizeFactureId(model.FactureId),
            model.Date.Date,
    ...
    private static List<CreateAvoirClientLigneDto> ToLineDtos(IEnumerable<AvoirLigneViewModel> lignes) =>
        lignes
            .Select(l => new CreateAvoirClientLigneDto(
                l.ProduitId,
                l.Designation.Trim(),
                l.Quantite,
                l.PrixUnitaireHT,
    ...
    private void ValidateLignes(AvoirFormViewModel model)
    {
