...
public sealed class DevisController(
    ...
    IClientService clients) : Controller
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await devis.GetDevisAsync(
            search,
            page,
            DevisListViewModel.DefaultPageSize,
            cancellationToken);

        return View(new DevisListViewModel
        {
            Items = result.Items,
    ...
    {
        var numero = await devis.GenerateNumeroAsync(cancellationToken);

        return View("Edit", new DevisFormViewModel
        {
            Numero = numero,
            Date = DateTime.Today,
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            await devis.CreateDevisAsync(ToCreateDto(model), cancellationToken);
        }
        ...
        TempData["Success"] = "Devis enregistré avec succès.";
    ...
    {
        var existing = await devis.GetDevisByIdAsync(id, cancellationToken);
        if (existing is null)
            return NotFound();
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            await devis.UpdateDevisAsync(id, ToUpdateDto(model), cancellationToken);
        }
    ...
    public async Task<IActionResult> Delete(int id, CancellationToken cancellationToken)
    {
        try
        {
            await devis.DeleteDevisAsync(id, cancellationToken);
    ...
    private async Task ResolveClientNomAsync(DevisFormViewModel model, CancellationToken cancellationToken)
    {
        if (model.ClientId <= 0)
            return;
    ...
    private async Task<DevisFormViewModel> ToFormViewModelAsync(
        DevisClientDto dto,
        CancellationToken cancellationToken)
    {
        var client = await clients.GetClientByIdAsync(dto.ClientId, cancellationToken);

        return new DevisFormViewModel
        {
            Id = dto.Id,
            Numero = dto.Numero,
            ...
            Note = dto.Note,
            Lignes = dto.Lignes.Select(l => new DevisLigneViewModel
            {
    ...
    private static CreateDevisClientDto ToCreateDto(DevisFormViewModel model) =>
        new(
            model.Numero.Trim(),
            model.ClientId,
            model.Date.Date,
            model.DateValidite.Date,
            model.RemiseGlobale,
            Normalize(model.Note) ?? string.Empty,
            ToLineDtos(model.Lignes));

    private static UpdateDevisClientDto ToUpdateDto(DevisFormViewModel model) =>
        new(
            model.ClientId,
            model.Date.Date,
            model.DateValidite.Date,
    ...
    private static List<CreateDevisClientLigneDto> ToLineDtos(IEnumerable<DevisLigneViewModel> lignes) =>
        lignes
            .Select(l => new CreateDevisClientLigneDto(
                l.ProduitId,
                l.Designation.Trim(),
                l.Quantite,
                l.PrixUnitaireHT,
    ...
    private void ValidateLignes(DevisFormViewModel model)
    {
